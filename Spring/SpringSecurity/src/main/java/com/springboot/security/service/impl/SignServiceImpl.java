...
import com.springboot.security.config.secuirty.JwtTokenProvider;
...
public class SignServiceImpl implements SignService {
    ...
    public SignUpResultDto signUp(String id, String password, String name, String role){
        ...
        if(role.equalsIgnoreCase("admin")){
            user=User.builder()
                    ...
                    .roles(Collections.singletonList("ROLE_ADMIM"))
                    .build();
        }else{
            user=User.builder()
                    .uid(id)
        ...
        LOGGER.info("[getSignUpResult] userEntity 값이 들어갔는지 확인후 결과괎 주입");
        if(!savedUser.getName().isEmpty()){
            LOGGER.info("[getSignUpResult] 정상 처리");
    ...
    public SignInResultDto signIn(String id, String password) throws RuntimeException{
        LOGGER.info("[getSignInReuslt] signDataHandler로 회원 정보 요청");
        User user=userRepository.getByUid(id);
        LOGGER.info("[getSignInReuslt] Id:{}",id);

        LOGGER.info("[getSignInResult] 패스워드 비교수행!");
        if(!passwordEncoder.matches(password,user.getPassword())){
            throw new RuntimeException();
        }
        LOGGER.info("[getSignInResult패스워드 일치");
